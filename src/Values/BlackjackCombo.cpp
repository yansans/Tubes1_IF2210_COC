...
{
    return std::count_if(cards.begin(), cards.end(), [](BlackjackCard c)
                         { return c.get_value() == 11; });
}
...
{
    std::function<int(BlackjackCard)> lambda = [](BlackjackCard c)
    { return (int)(c.get_value()); };
